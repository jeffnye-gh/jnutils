...
// -------------------------------------------------------------------------
void TcpServer::publishClientMsg(const Client & client, const char * msg, 
                                 size_t msgSize)
{
  for (uint i=0; i<m_subscibers.size(); i++) {
    if (   m_subscibers[i].wantedIp == client.getIp()
        || m_subscibers[i].wantedIp.empty())
    {
...
pipe_ret_t TcpServer::start(int port) {
  ...
  // set socket for reuse (otherwise might have to wait 4 minutes 
  // every time socket is closed)
  int option = 1;
  ...
  int bindSuccess = bind(m_sockfd, (struct sockaddr *)&m_serverAddress,
